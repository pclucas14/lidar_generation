...
from utils import * 
from models import * 
from hyperspherical import HypersphericalUniform as HU
...
dis = netD(args, ndf=64, nc=6 if args.no_polar else 4, lf=(2,16)).cuda()
...
# since now we are processing pairs of images, we need to filter out invalid pairs.
diff  = ((dataset[1:] - dataset[:-1]) ** 2).sum(axis=-1).sum(axis=-1).sum(axis=-1)
keep  = diff < np.percentile(diff, 99.5)
pairs = np.stack([dataset[1:], dataset[:-1]], axis=1)
print('discarding {} pairs out of {}'.format((1 - keep).sum(), keep.shape[0]))

# data        = from_polar(torch.cuda.FloatTensor(dataset)).cpu().data.numpy()
# pairs_xyz   = np.stack([data[1:], data[:-1]], axis=1)
# dataset_xyz = pairs_xyz[keep == 1]

dataset = pairs[keep == 1]
dataset = dataset.reshape(dataset.shape[0], -1, *dataset.shape[3:])
...
for epoch in range(2500):
    ...
    while iters < len(loader):
        ...
        while j < args.dis_iters and iters < len(loader):
            ...
            noise_a, noise_b = noise_dist.sample_pair(input.size(0))
            fake = torch.cat([gen(noise_a), gen(noise_b)], dim=1)
            fake_out = dis(fake)
